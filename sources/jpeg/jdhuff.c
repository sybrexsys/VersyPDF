...
#define savable_state decompress_savable_state
...
jpeg_fill_bit_buffer (bitread_working_state * state,
		      ...
		      int nbits, j_stream_ptr from_memory)
/* Load up the bit buffer to a depth of at least nbits */
{
  ...
  if (cinfo->unread_marker == 0) {	/* cannot advance past a marker */
	...
	if (! (*cinfo->src->fill_input_buffer) (cinfo, from_memory))
	  return FALSE;
...
jpeg_huff_decode (bitread_working_state * state,
		  register bit_buf_type get_buffer, register int bits_left,
		  d_derived_tbl * htbl, int min_bits, j_stream_ptr from_memory)
{
...
METHODDEF(boolean)
decode_mcu (j_decompress_ptr cinfo, JBLOCKROW *MCU_data, j_stream_ptr from_memory)
{
  huff_entropy_ptr entropy = (huff_entropy_ptr) cinfo->entropy;
  int blkn;
  BITREAD_STATE_VARS;
