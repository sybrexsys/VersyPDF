...
TIFFFdOpen(int ifd, const char* name, const char* mode)
{
	...
	tif = TIFFClientOpen(NULL, name, mode, (thandle_t)ifd,
			_tiffReadProc, _tiffWriteProc,
			_tiffSeekProc, _tiffSizeProc, _tiffCloseProc,  NULL, NULL, NULL);
	if (tif)
		tif->tif_fd = ifd;
	return (tif);
}
...
tdata_t
_TIFFmallocA(tsize_t s)
{
	return ((tdata_t)GlobalAlloc(GMEM_FIXED, s));
}

void
_TIFFfreeA(tdata_t p)
{
	GlobalFree(p);
	return;
}
...
tdata_t
_TIFFreallocA(tdata_t p, tsize_t s)
{
        void* pvTmp;
        tsize_t old;
