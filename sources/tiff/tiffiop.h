...
/* tag support */
    ...
    TIFFMallocProc  tif_mallocproc;
    TIFFReallocProc tif_reallocproc;
    TIFFFreeProc tif_freeproc;  
    tdata_t lib;
...
#define	TIFFCloseFile(tif) \
    ((*(tif)->tif_closeproc)?(*(tif)->tif_closeproc)((tif)->tif_clientdata):0)
...
#define	TIFFMalloc(tif, size) \
    ((*(tif)->tif_mallocproc) ? (*(tif)->tif_mallocproc)(tif, size): _TIFFmallocA( size ) )
#define	TIFFRealloc(tif, buffer, size) \
    ((*(tif)->tif_reallocproc) ? (*(tif)->tif_reallocproc)(tif, buffer, size): _TIFFreallocA( buffer, size ) )
#define	TIFFFree(tif, buffer) \
    ((*(tif)->tif_freeproc) ? (*(tif)->tif_freeproc)(tif, buffer): _TIFFfreeA( buffer ) )

#define _TIFFfree( buffer ) TIFFFree ( tif, buffer )
#define _TIFFmalloc( size ) TIFFMalloc ( tif, size )
#define _TIFFrealloc( buffer, size ) TIFFRealloc ( tif, buffer, size )
