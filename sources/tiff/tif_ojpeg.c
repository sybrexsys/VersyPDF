...
TIFFojpeg_read_header(register OJPEGState *sp,boolean require_image)
  {return CALLJPEG(sp,-1,jpeg_read_header(&sp->cinfo.d,require_image, NULL));}
...
TIFFojpeg_read_scanlines(register OJPEGState *sp,JSAMPARRAY scanlines,
                        int max_lines)
  { return
      CALLJPEG(sp,-1,(int)jpeg_read_scanlines(&sp->cinfo.d,scanlines,(JDIMENSION)max_lines, NULL));
  }
...
static boolean
std_empty_output_buffer(register j_compress_ptr cinfo, j_stream_ptr mem_stream)
  {
...
static void
std_term_destination(register j_compress_ptr cinfo, j_stream_ptr from_memory)
  {
#   define sp ((OJPEGState *)cinfo)
    register TIFF *tif = sp->tif;
...
static void
tables_term_destination(register j_compress_ptr cinfo, j_stream_ptr from_memory)
  {
#   define sp ((OJPEGState *)cinfo)
...
static boolean
std_fill_input_buffer(register j_decompress_ptr cinfo, j_stream_ptr from_memory)
...
static void
std_skip_input_data(register j_decompress_ptr cinfo,long num_bytes, j_stream_ptr from_memory)
  {
#   define sp ((OJPEGState *)cinfo)

    if (num_bytes > 0)
      if (num_bytes > (long)sp->src.bytes_in_buffer) /* oops: buffer overrun */
        (void)std_fill_input_buffer(cinfo, NULL);
      else
        {
...
fake_SOS_marker(j_decompress_ptr cinfo, j_stream_ptr from_memory){return JPEG_REACHED_SOS;}

/*ARGSUSED*/ METHODDEF(int)
suspend(j_decompress_ptr cinfo, j_stream_ptr from_memory){return JPEG_SUSPENDED;}
...
#       endif /* someday */
     ...
     */
     ...
     L: if (p - (unsigned char *)sp->jpegtables > 2) /* fake "JPEGTables" */
      ...
      { int (*save)(j_decompress_ptr cinfo, j_stream_ptr mem_stream ) = sp->cinfo.d.marker->read_markers;
        register int i;
