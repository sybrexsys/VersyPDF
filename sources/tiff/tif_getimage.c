...
initYCbCrConversion(TIFFRGBAImage* img)
{
    ...
    if (img->ycbcr == NULL) {
	img->ycbcr = (TIFFYCbCrToRGB*) TIFFMalloc( img->tif, 
...
makebwmap(TIFFRGBAImage* img)
{
    ...
    img->BWmap = (uint32**) TIFFMalloc( img->tif,
...
setupMap(TIFFRGBAImage* img)
{
    ...
    img->Map = (TIFFRGBValue*) TIFFMalloc( img->tif,(range+1) * sizeof (TIFFRGBValue));
    if (img->Map == NULL) {
	TIFFError(TIFFFileName(img->tif),
	...
	TIFFFree(img->tif, img->Map), img->Map = NULL;
    }
    return (1);
}
...
makecmap(TIFFRGBAImage* img)
{
    ...
    img->PALmap = (uint32**) TIFFMalloc( img->tif,
	256*sizeof (uint32 *)+(256*nsamples*sizeof(uint32)));
    if (img->PALmap == NULL) {
